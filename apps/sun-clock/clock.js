...
function drawMinuteAgeFadedPath(ray, marker, progress) {
  ...
  const midOpacity = 0.66;
  const lowOpacity = 0.33;
  ...
  for (let chunk = 0; chunk < chunks; chunk += 1) {
    ...
    } else if (ageMinutes <= 4.5) {
      const transition = smoothStep(3, 4.5, ageMinutes);
      opacity = midOpacity - transition * (midOpacity - lowOpacity);
    } else {
      const tailMinutes = Math.max(1, currentMinutes - 4.5);
      const tailProgress = smoothStep(0, 1, Math.min(1, (ageMinutes - 4.5) / tailMinutes));
      opacity = MINUTE_INACTIVE_ALPHA + (lowOpacity - MINUTE_INACTIVE_ALPHA) * (1 - tailProgress);
    }
